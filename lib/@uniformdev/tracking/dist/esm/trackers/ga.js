import { GaDispatcher } from "../connectors/ga/dispatcher";
import { getGaTrackedActivityConverterForSitecore } from "../connectors/sitecore/gaEventConverter";
export function getDispatchersForGaDestinations(destinations, args) {
...
function getDispatcher(destination, args) {
    ...
    var dispatcher = new GaDispatcher(converters, destination);
...
function getActivityConverters(destination, args) {
    ...
    if (destination.doNotUseDefaultActivityConverter != true) {
        var converter_1 = getGaTrackedActivityConverterForSitecore();
        if (converter_1) {
