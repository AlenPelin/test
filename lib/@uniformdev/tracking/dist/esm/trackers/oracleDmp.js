import { getOracleDmpCallbackName, OracleDmpDispatcher } from "../connectors/oracleDmp/dispatcher";
import { getOracleDmpTrackedActivityConverterForSitecore } from "../connectors/sitecore/oracleDmpEventConverter";
export function getDispatchersForOracleDmpDestinations(destinations, args) {
...
function getDispatcher(destination, args) {
    ...
    var dispatcher = new OracleDmpDispatcher(converters, destination);
    return dispatcher;
}
...
function getActivityConverters(destination, args) {
    ...
    if (destination.doNotUseDefaultActivityConverter != true) {
        var converter_1 = getOracleDmpTrackedActivityConverterForSitecore();
        if (converter_1) {
