...
describe('Visitor', function () {
    it('getLatestVisit returns undefined when no visits are provided.', function () {
        var visitor = new visitor_1.Visitor('111');
    ...
    it('getLatestVisit returns Visit when at least one visit is provided.', function () {
        ...
        visitor.visits = [new visit_1.Visit('AAA', '111', start)];
        var visit = utils_1.getLatestVisit(visitor);
    ...
    it('getLatestVisit returns latest Visit when multiple visits are provided regardless of order.', function () {
        ...
        var visits1 = [
            new visit_1.Visit('AAA', '111', start1, { updated: new Date(1).toISOString() }),
            new visit_1.Visit('BBB', '111', start1, { updated: new Date(2).toISOString() }),
        ];
        var start2 = new Date(2020, 2, 3).toISOString();
        var visits2 = [
            new visit_1.Visit('BBB', '111', start2, { updated: new Date(2).toISOString() }),
            new visit_1.Visit('AAA', '111', start2, { updated: new Date(1).toISOString() }),
        ];
        var visitor = new visitor_1.Visitor('111');
        visitor.visits = visits1;
        var visit1 = utils_1.getLatestVisit(visitor);
        ...
        var visit2 = utils_1.getLatestVisit(visitor);
