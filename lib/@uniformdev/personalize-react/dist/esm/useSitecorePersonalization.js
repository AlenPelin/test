 ...
 * @param props
 */
export function useSitecorePersonalization(props) {
    var _a;
    //
    //
    ...
    const personalizationContext = useContext(SitecorePersonalizationContext);
    const { personalizationManager, item } = personalizationContext;
    //
    //
    const { fields: nonPersonalizedFields, logger = (_a = personalizationContext.logger) !== null && _a !== void 0 ? _a : getNullLogger(), rendering, track = false, } = props;
    //
    //
    const { uid } = rendering;
    //
    //
    const dsInitial = tryFormatGuid(rendering.dataSource, 'D');
    //
    //
    ...
    useEffect(() => {
        ...
        if (track == true) {
            ...
            (_d = (_c = window.uniform) === null || _c === void 0 ? void 0 : _c.queue) === null || _d === void 0 ? void 0 : _d.add(QUEUE_ENTRY_TYPE_TRACKER, entry);
            //
            //
            logger.debug('useSitecorePersonalization - Entry was added to global queue.', { type: QUEUE_ENTRY_TYPE_TRACKER, entry });
        }
    }, [state.ds]);
