...
import { getSitecoreTracker, getCookie, setCookie, removeCookie, UniformCookieNames, getTrackerCookieTypes, addSubscriptionsForTrackerCookies, getDispatchersFromTrackingConfig } from '@uniformdev/tracking';
import { getNullLogger, getSubscriptionManager } from '@uniformdev/common';
import { getClientScriptLoader } from '@uniformdev/common-client';
var debugLogger = {
...
function getLogger(settings) {
    return (settings === null || settings === void 0 ? void 0 : settings.debug) == true ? debugLogger : getNullLogger();
}
...
function addSubscriptionsForContextCookies(trackingConfig, subs, logger) {
    var cookieTypes = getTrackerCookieTypes(trackingConfig);
 ...
 * @param settings
 */
export function initializeTracker(settings) {
    var _a, _b, _c;
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    ...
                    scriptLoader = getClientScriptLoader();
                    if (!scriptLoader) {
                ...
                case 4:
                    ...
                    dispatchers = getDispatchersFromTrackingConfig(trackingConfig, {
                        getCookie: getCookie,
                        logger: logger,
                        loggerPrefix: "Uniform tracking - initializeTracker",
                        removeCookie: removeCookie,
                    ...
                    tracker = getSitecoreTracker(args, logger);
                    if (!tracker) {
 ...
 * @param settings
 */
export function doTracking(settings) {
    var _a;
    ...
    var useTracker = function (tracker) {
        ...
        var visitorId = getCookie(UniformCookieNames.VisitorId);
        logger.debug("Uniform tracking - doTracking - Visitor id was retrieved from cookie.", { visitorId: visitorId, cookie: UniformCookieNames.VisitorId });
        ...
        if (results && results.visitor) {
            setCookie(UniformCookieNames.VisitorId, results.visitor.id);
            logger.debug("Uniform tracking - doTracking - Visitor id cookie was updated.", { visitorId: visitorId, cookie: UniformCookieNames.VisitorId });
        }
    };
